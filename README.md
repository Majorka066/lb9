# 📘 README.md — Каталог фильмов (Movie Search)

## 📌 Название проекта
**Мини-проект: Каталог фильмов (Movie Search)**  
React-приложение для поиска фильмов через OMDb API.

## 🌐 Пример использованного API-запроса

Поиск фильмов по названию:
```
https://www.omdbapi.com/?apikey=73eff1f9&s=avatar
```

Загрузка подробной информации по ID фильма:
```
https://www.omdbapi.com/?apikey=73eff1f9&i=tt0499549
```

## 🖼 Скриншоты работы приложения

<img width="1907" height="903" alt="image" src="https://github.com/user-attachments/assets/4055f0dd-e701-4a7f-9d74-111fa7b92bda" />
<img width="1905" height="909" alt="image" src="https://github.com/user-attachments/assets/dfb413f2-04a3-4aed-8d44-099c105f5297" />
<img width="1898" height="905" alt="image" src="https://github.com/user-attachments/assets/b25bc264-714a-48aa-b49a-21b1e91bbaf9" />

# 🧠 Ответы на вопросы

## 1. Что делает useEffect в вашем приложении?

React — это библиотека для создания интерфейсов (UI) на JavaScript.

useEffect — Это специальное место для кода, который должен выполняться 
не во время рисования, а после него, и при определенных условиях.

В моём приложении `useEffect` отвечает за отправку запроса к API OMDb каждый раз, когда изменяется состояние `query` (строка поиска). Он загружает список фильмов и обновляет состояние приложения.

Важно: Саму функцию useEffect нельзя делать async. React ожидает, что эффект вернет функцию очистки или ничего, а
async функция возвращает Promise.

Promise — это объект в JavaScript, который представляет результат асинхронной операции, которая:
выполняется не сразу,
займёт какое-то время,
и завершится успешно или с ошибкой.

API — это способ, с помощью которого одна программа может общаться с другой.

React Hooks (хуки) — это специальные функции в React, которые позволяют:
- добавлять логику и состояние в функциональные компоненты.
<img width="590" height="315" alt="image" src="https://github.com/user-attachments/assets/612130b0-e71f-499c-91ce-a381d0923962" />

## 2. Какие состояния вы использовали и зачем?

| Состояние | Назначение |
|----------|------------|
| `query` | Строка запроса пользователя |
| `movies` | Массив найденных фильмов |
| `selectedMovie` | Фильм, выбранный для модального окна |
| `loading` | Индикатор загрузки данных |
| `error` | Сообщение об ошибке API |

## 3. Где ИИ помог, а где пришлось разбираться самому?

### 🤖 ИИ помог: 
- генерации компонентов  
- объяснении работы useEffect  
- оптимизации API-запросов  
- написании UI и модального окна  

### 🧑‍💻 Сам разбирался:
- подключение API-ключа  
- исправление ошибок  
- изучение документации

## 4. Что из документации API было важнее всего?

- Правильный формат запросов (`s=` и `i=`)  
- Обязательное наличие `apikey`  
- Формат ответа JSON  
- Обработка ошибок через `Response: "False"`  
